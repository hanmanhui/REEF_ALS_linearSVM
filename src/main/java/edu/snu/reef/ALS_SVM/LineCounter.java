	...
	public class ContextActiveHandler implements EventHandler<ActiveContext> {
		...
		public void onNext(final ActiveContext activeContext) {
			...
			if (dataLoadingService.isDataLoadedContext(activeContext)) {
				...
				LOG.log(Level.FINEST, "Submit LineCount context {0} to: {1}", new Object[] { lcContextId, contextId });
				...
				activeContext.submitContext(
					Tang.Factory.getTang().newConfigurationBuilder(
							...
							.set(ContextConfiguration.IDENTIFIER, lcContextId)
							.build()
						)
						.build()
					);
				...
				try {
					activeContext
							.submitTask(TaskConfiguration.CONF
